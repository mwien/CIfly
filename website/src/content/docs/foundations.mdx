...
In this article, we explore the main ideas underlying the CIfly framework. This will lay the groundwork for introducing the precise CIfly syntax afterwards. For a more detailed exposition, we refer to the [paper](https://arxiv.org/abs/2506.15758) which thoroughly develops these theoretical foundations. 
...
For more details on this example, see our [paper](https://arxiv.org/abs/2506.15758). In the next articles, we will focus on the syntax of CIfly rule tables and the ```ciflypy``` and ```ciflyr``` packages for using CIfly in your Python or R code. 
