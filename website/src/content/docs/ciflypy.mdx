...
The ```ciflypy``` graph format is based on edge lists for different graph types. Graphs can usually be converted to this format with little effort. For example, consider a directed graph in ```networkx```:
```py
...
NumPy adjacency matrices can be converted just as easily. Consider the following matrix representing the same directed graph again:
```py
import numpy as np
