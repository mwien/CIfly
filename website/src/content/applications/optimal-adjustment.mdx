...
`anc <- reach(g, list("X" = Y, "W" = X), "./possible_ancestors_cpdag.txt")
des <- reach(g, list("X" = X, "W" = c()), "./possible_descendants_cpdag.txt")
cn <- intersect(anc, des)
forb <- reach(g, list("X" = cn), "./possible_descendants_cpdag.txt")`
} lang="r" title="" />
</CodeSwitcher>
