...
summary: Testing whether a set of variables can be used for adjustment given a CPDAG. 
code: [cpdag_adjustment_check.py]
...
<Ruletable id="not_amenable_cpdag"/>
...
<Ruletable id="possible_ancestors_cpdag"/>

Using these tables (assuming they are stored in ```possible_anc``` and ```possible_des```), the forbidden set can be computed as follows.
...
<Ruletable id="backdoor_connected_cpdag"/>
...
<ExampleCode id="cpdag_adjustment_check.py"/>
