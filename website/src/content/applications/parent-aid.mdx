...
With these sets we can compote the parent adjustment distance directly as defined in Definition 1. For this, we iterate over all $y$ and check whether the parent adjustment distances increases for pair $x$ and $y$ in constant time per $y$. Overall, this yields run-time $O(p \cdot (p + m))$. 
