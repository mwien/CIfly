...
- all $y$ such that the pair $x$ and $y$ is not amenable in $G_{\text{true}}$
- all $y$ such that $\text{pa}_{G_{\text{guess}}}(x)$ is not a valid adjustment set relative to $x$ and $y$ in $G_{\text{true}}$. 

Based on these information the parent adjustment distance can be computed directly based on Definition 1. For this, we iterate over all $y$ and check whether the parent adjustment distances increases for pair $x$ and $y$ in constant time per $y$. Overall, this yields run-time $O(p \cdot (p + m))$. 
