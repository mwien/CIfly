...
The parent adjustment distance measures the distance of $G_{\text{guess}}$ from $G_{\text{true}}$. Broadly speaking (we will define this more precisely below), this distance measures the number of node pairs $x$ and $y$ for which the validity of the adjustment set consisting of the parents of $x$ in $G_{\text{guess}}$ differs between the two graphs. E.g., if the parents of $x$ in $G_{\text{guess}}$ are an adjustment set for $G_{\text{guess}}$ but not for $G_{\text{true}}$ the distance increases by one. To make this more precise, let us recall the CPDAG adjustment criterion by <Reference id="perkovicComplete" format="author(year)"/> which we discussed previously in [this article](/applications/cpdag-adjustment-check/). 
...
Consider a CPDAG $G$ and sets of nodes $X$, $Y$ and $Z$. Then, $Z$ satisfies the adjustment criterion if, and only if,

1. every proper possibly directed walk from $X$ to $Y$ begins with a directed edge out of $X$, 
2. no proper possibly directed walk from $X$ to $Y$ contains a node in $Z$ and
3. every proper definite-status walk from $X$ to $Y$ that contains a backwards-facing edge is blocked by $Z$. 

Algorithmically, it is easier to check the complements of the first and the third condition, that is, to check that no proper possibly directed walk from $X$ to $Y$ begins with an *undirected* edge and that no proper definite-status walk from $X$ to $Y$ that contains a backwards-facing edge is *open* given $Z$. 
...
- *open* if every collider is in $Z$ and every non-collider is not in $Z$ and
...
For the second condition, we need to track proper possibly directed walks from $X$ that contain a node in $Z$. 

<Ruletable id="forbidden_path_connected_cpdag" withHeader={true}/>

For properness, we always check that the ```next``` node is not in $X$. Moreover, we switch to color ```yield``` once we find a node that is in $Z$ while only tracking possibly directed paths. Hence, the set of nodes of color ```yield``` that is returned contains all nodes reachable from $X$ via walks prescribed by Condition 2 of Lemma 1.

Tracking the paths in Condition 3 is a bit more complex. We leave the ```init``` color with the first edge. Afterwards, open paths are tracked, which may become non-causal if a backwards facing edge is traversed. For every rule, properness is ensured by checking that ```next``` is not in $X$. The further logic in the rules stems from verifying that the walk is open and definite-status. The nodes visited with color ```non-causal``` are returned. 

<Ruletable id="non_causal_connected_cpdag" withHeader={true}/>

### CIfly-based computation of the parent adjustment distance
The general idea of the algorithm is to go through all $x$ (in the following, we write $t$ standing for *treatment* instead of $x$), look up its parents in $G_{\text{guess}}$ (denoted by $\text{pa}_{G_{\text{guess}}}(t)$) and then compute the following sets in linear-time:

- all $y$ such that the pair $t$ and $y$ is not amenable (violates Condition 1 of the adjustment criterion) in $G_{\text{guess}}$
- all $y$ such that $y$ is a proper possible descendant of $t$
- all $y$ such that the pair $t$ and $y$ is not amenable in $G_{\text{true}}$
- all $y$ such that $\text{pa}_{G_{\text{guess}}}(x)$ is not a valid adjustment set relative to $t$ and $y$ in $G_{\text{true}}$. 

Based on these information the parent adjustment distance can be computed directly based on Definition 1. For this, we iterate over all $y$ and check whether the parent adjustment distances increases for pair $t$ and $y$ in constant time per $y$. Overall, this yields run-time $O(p \cdot (p + m))$. 
...
- we parse the graphs into CIfly objects outside of the loop over all $t$,
- we have a *single* function which returns not-amenable $y$ and all $y$ for which the adjustment set is not valid in $G_{\text{guess}}$ to avoid computing non-amenable $y$ twice and
