...
The resulting sets $Z$ and $W$ are valid, provided $Z$ is non-empty, which is trivial to check outside of CIfly. The other condition is that $Z$ contains a parent or sibling of $x$. While this could also be checked using CIfly, clearly it is preferred to do this directly in R. Here, we do this based on the CIfly graph representation in R, but you may prefer to do this using your favorite graph library or format instead.

Recall that a CIfly graph is stored as one edge list per edge type (as explained [here](/docs/r-wrapper/)). The edge lists are given as $r \times 2$ matrices in case there are $r$ edges of that type. In order to check whether some node in $Z$ is a parent of $x$ we can filter the edge list matrix for such rows. For efficient look-up in the filtering, we first store a Boolean vector indicating membership in $Z$. Afterwards, we filter the edges in ```g[["-->"]]``` that have a node in $Z$ as first and $x$ as second element and return ```TRUE``` or ```FALSE``` based on the length of the filtered matrix. 
...
This code uses a small helper function ```highestVertexCifly``` to find the highest node ID in a CIfly graph. Note that the number of nodes is not stored in a CIfly graph object and, hence, has to be managed outside of it if needed. 
...
Both functions take time $O(m)$ when the CIfly graph has $m$ edges. This is fine in this case because they are called only once. If they are needed repeatedly for different nodes, then it is certainly useful to maintain a different graph representation, in addition to the CIfly graph, with more efficient access to these information. 

The siblings can be retrieved similarly, only this time checking for both possible orders of the endpoints in the edge list matrix. 
...
In our code these functions can be used as follows to give a Boolean checking Condition 2. 
```r
containsParentCifly(g, x, Z) || containsSiblingCifly(g, x, Z)
```

In case this is needed more often, a helper function 
```r 
containsNeighborCifly(g, x, Z, edgeType, orderedEdge=TRUE)
```
may prove useful that abstracts this code for different edge types including an argument that indicates whether this edge type is ordered.  
