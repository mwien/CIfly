...
**Theorem** [<Reference id="henckelIV" format="author(year)"/>]    
...
While using the latent projection over descendants of $x$ (apart from $y$) is correct under this assumption, algorithmically, computing the latent projection is more expensive than CIfly reachability algorithms. Hence, we would like to avoid this operation here. To this aim, we use the following graphical formulation for $W^\text{opt}$ and $Z^\text{opt}$ in the CIfly paper. 

**Theorem**    
Consider distinct nodes $x$ and $y$ in an ADMG $G$ such that $y \in \text{de}_G(x)$ and let $A = V \setminus D$. Moreover, define
