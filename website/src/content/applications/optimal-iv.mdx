...
The resulting sets $Z$ and $W$ are valid, provided $Z$ is non-empty, which is trivial to check outside of CIfly. The second condition is that $Z$ contains a parent or sibling of $x$. While this could also be checked using a CIfly algorithm, clearly it is preferred to do this directly in Python or R. Here, we do this based on the CIfly graph representation, but you may prefer to do this using your favorite graph library or format instead.
...
This code uses a small helper function ```highestNodeId``` to find the highest node ID in a CIfly graph. Note that the number of nodes is not stored in a CIfly graph object and, hence, has to be managed outside of it if needed. 
...
Again, the siblings can be retrieved similarly, only now checking for both possible orders of the endpoints in the edge list matrix. 
