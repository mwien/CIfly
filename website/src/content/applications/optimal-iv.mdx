...
While using the latent projection to removde all descendants of $x$ (apart from $y$) is methodologically reasonable under this assumption, algorithmically, computing the latent projection is more expensive than CIfly reachability algorithms. Hence, we would like to avoid this operation here. To this aim, we use the following graphical characterization of $W^\text{opt}$ and $Z^\text{opt}$ developed in the CIfly paper. 
